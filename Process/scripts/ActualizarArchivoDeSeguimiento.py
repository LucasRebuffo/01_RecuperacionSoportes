    ...
    try:
        ...
        for i, header in enumerate(normalized_headers):
            column_mapping[header] = (
                i + 1
            )  # +1 porque openpyxl usa indices basados en 1
        ...
        for row in range(2, worksheet.max_row + 1):
            empresa_value = worksheet.cell(
                row=row, column=column_mapping.get("Empresa", 1)
            ).value
        ...
        clave_buscar = (
            str(input_data["Tipo Docto"])
            + "_"
            + str(input_data["Nro Docto"])
            + "_"
            + str(input_data["Documento Cruce"])
        )
        ...
        for row in range(2, worksheet.max_row + 1):
            tipo_docto_value = worksheet.cell(
                row=row, column=column_mapping.get("Tipo Docto", 1)
            ).value
            nro_docto_value = worksheet.cell(
                row=row, column=column_mapping.get("Nro Docto", 1)
            ).value
            documento_cruce_value = worksheet.cell(
                row=row, column=column_mapping.get("Documento Cruce", 1)
            ).value

            if (
                tipo_docto_value is not None
                and nro_docto_value is not None
                and documento_cruce_value is not None
            ):
                clave_actual = (
                    str(tipo_docto_value)
                    + "_"
                    + str(nro_docto_value)
                    + "_"
                    + str(documento_cruce_value)
                )
        ...
        if len(filas_encontradas) == 0:
            ...
            for idx, clave in enumerate(
                claves_encontradas[:10]
        ...
        logger.info(
            f"Se encontraron {len(filas_encontradas)} filas con la clave compuesta: {clave_buscar}"
        )
...
resultado = actualizar_fila_especifica(
    input_data, "{gblRutaInput}" + "/Archivo de Seguimiento.xlsx"
)
